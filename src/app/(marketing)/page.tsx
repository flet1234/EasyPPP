...
import { formatCompactNumber } from "@/lib/formaters";
import { cn } from "@/lib/utils";
import { SignUpButton } from "@clerk/nextjs";
import { link } from "fs";
import { ArrowRightIcon, CheckIcon, IceCream } from "lucide-react";
    ...
    <>
      <section className="min-h-screen bg-[radial-gradient(hsl(0,72%,65%,40%),hsl(24,62%,73%,40%),hsl(var(--background))_60%)] flex items-center justify-center text-center text-balance flex-col gap-8 px-4">
        ...
        <p className="text-lg lg:text-3xl max-w-screen-xl">
          Optimize your product pricing across countries to maximize sales. Capture 85% of the untapped market with location-based dynamic
          pricing
        </p>
        <SignUpButton>
      ...
      <section className="bg-primary text-primary-foreground">
        <div className="container py-16 flex flex-col gap-16 px-8 md:px-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-16">
            <Link href="#">
              <IceCream className="size-20 mx-auto" />
            </Link>
            <Link href="#">
              <IceCream className="size-20 mx-auto" />
            </Link>
            ...
            <Link href="#" className="md:max-xl:hidden mx-auto">
              <IceCream className="size-20" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <footer className="container pt-16 pb-8 flex sm:flex-row gap-8 sm:gap-4 justify-between items-start">
        <Link href="/">
          <BrandLogo />
        </Link>
        <div className="flex flex-col sm:flex-row gap-8">
          <div className="flex flex-col gap-8">
            ...
            <FooterLinkGroup
              title="Solutions"
              links={[
                { label: "Sell me salutary", href: "#" },
                { label: "Brother of the father", href: "#" },
                { label: "Pollutions", href: "#" }
              ]}
            />
          </div>
          <div className="flex flex-col gap-8">
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>{formatCompactNumber(maxNumberOfVisits)} pricing page visist/mo</CardDescription>
        </CardHeader>
        <CardContent>
          <SignUpButton>
